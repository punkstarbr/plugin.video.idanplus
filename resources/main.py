...
def LiveChannels():
    ...
    for channel in channels:
        if channel.get('type') == 'refresh':
            ...
            common.addDir(name, 'Container.Refresh', channel['mode'], channel['image'], infos={"Title": name},
                          moreData=';noexit', isFolder=False)
        else:
            programs = [] if channel['tvgID'] == '' else nowEPG.get(channel['tvgID'], [])
            LiveChannel(common.GetLocaleString(channel['nameID']), channel['channelID'], channel['mode'],
                        channel['image'], channel['module'], contextMenu=[], resKey=channel['resKey'],
                        programs=programs, tvgID=channel['tvgID'])
...
def VODs():
	...
	common.addDir(name, 'https://www.kan.org.il/lobby/digital-lobby', 1, common.GetIconFullPath("23tv.jpg"), infos={"Title": name}, module='kan')
	name = common.GetLabelColor('{0} - Site antigo'.format(common.GetLocaleString(30607)), bold=True, color="none")
...
def Search():
	...
	filteredSeries = []
	seriesLinks = []
	searchText = common.GetKeyboardText('Digite as palavras para pesquisa', '').strip().lower()
	if searchText != '':
		for serie in series:
